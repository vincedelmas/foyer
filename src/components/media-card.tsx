...
export function MediaCard({ item, index = 0 }: { item: MediaSummary, index?: number }) {
    const poster = tmdbImage(item.posterPath, "w500");

    return (
        <article data-archive-item className="group min-w-0" style={{ "--archive-index": index } as React.CSSProperties}>
            <Link to="/media/$id" params={{ id: item.id }} className="flex flex-col gap-3 outline-none">
                <div
                    ...
                    group-hover:-translate-y-1.5 group-hover:ring-primary/60 group-focus-visible:ring-2 group-focus-visible:ring-ring">
                    {poster ?
                        <img
                            alt=""
                            src={poster}
                    ...
                    <span className="absolute right-3 bottom-3 grid size-10 translate-y-2 place-items-center rounded-full bg-primary
                    text-primary-foreground opacity-0 shadow-lg transition group-hover:translate-y-0 group-hover:opacity-100">
