...
const configuredOrigin = process.env.FOYER_CORS_ORIGIN;


afterEach(() => {
    if (configuredOrigin === undefined) delete process.env.FOYER_CORS_ORIGIN;
    else process.env.FOYER_CORS_ORIGIN = configuredOrigin;
});


describe("CORS headers", () => {
    it("disables cross-origin browser access by default and for wildcards", () => {
        delete process.env.FOYER_CORS_ORIGIN;
        expect(corsHeaders()).toEqual({});

        process.env.FOYER_CORS_ORIGIN = "*";
        expect(corsHeaders()).toEqual({});
    });

    it("allows one explicitly configured browser origin", () => {
        process.env.FOYER_CORS_ORIGIN = "http://192.168.1.10:5173";
